...
import {
  AtlpPerfectScrollbarGeometry,
  AtlpPerfectScrollbarPosition,
} from './atlp-perfect-scrollbar.interfaces';
import { AtlpConfigService } from 'projects/@atlp/services/config.service';

@Directive({
  selector: '[atlpPerfectScrollbar]',
})
export class AtlpPerfectScrollbarDirective
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  @Input()
  set atlpPerfectScrollbarOptions(value) {
    // Merge the options
  ...
  get atlpPerfectScrollbarOptions(): any {
    // Return the options
  ...
  ngAfterViewInit(): void {
    ...
    if (this.atlpPerfectScrollbarOptions.updateOnRouteChange) {
      this._router.events
        .pipe(
  ...
  _init(): void {
    ...
    this.ps = new PerfectScrollbar(this.elementRef.nativeElement, {
      ...this.atlpPerfectScrollbarOptions,
    });
  ...
  geometry(prefix: string = 'scroll'): AtlpPerfectScrollbarGeometry {
    return new AtlpPerfectScrollbarGeometry(
  ...
  position(absolute: boolean = false): AtlpPerfectScrollbarPosition {
    if (!absolute && this.ps) {
      return new AtlpPerfectScrollbarPosition(
        this.ps.reach.x || 0,
        this.ps.reach.y || 0
      );
    } else {
  ...
  scrollToElement(
    ...
    let element;
    if (isId) {
      element = this.elementRef.nativeElement.querySelector(qs);
    } else {
      element = qs;
    }
