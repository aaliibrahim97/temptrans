...
@UntilDestroy()
@Component({
  selector: 'app-payment-review-methods',
...
export class PaymentReviewMethodsComponent implements OnInit, OnDestroy {
  ...
  @Input() paymentSliderKey: SidebarName = SidebarName.choosePaymentMethod;
  ...
  translatepaymentMethods(): void {
    ...
    // let crediCard = 'CREDIT / DEBIT CARD';
    // let internetBanking = 'INTERNET BANKING';
    // if (lang === 'ar' || lang === 'ae') {
    //   crediCard = 'بطاقة ائتمان';
    //   internetBanking = 'الدفع عبر الإنترنت';
    // }
        ...
        (paymentCheckoutResponseModel: PaymentCheckoutResponseModel) => {
          if (paymentCheckoutResponseModel.success) {
            ...
            if (!this.checkPopUpWindowforPaymentMethod(this.paymentData)) {
              ...
              this.toggleSidebarOpen(SidebarName.paymentCheckout);
            } else {
  ...
  onMessage(evt): void {
    if (evt.data.BankStatus) {
      // console.log("POSTMESSAGE_MPAY.API",evt);
      const evtData: AtlpPaymentResponseModel = {
        data: {
          OrderNumber: this.paymentReceiveModel.mpayPayload.OrderNumber,
          OnlineRefID: evt.data.OnlineRefID,
          TotalAmount: evt.data.TotalAmount,
          BankStatus: evt.data.BankStatus,
          TransRefID: this.paymentData.transRefId,
          PaymentMethod: this.paymentData.paymentMethod,
        },
        metaData: [evt],
      ...
      if (
        evt.data.BankStatus === PaymentBankStatus.AUTHORIZED ||
        evt.data.BankStatus === PaymentBankStatus.SUCCESS
      ) {
