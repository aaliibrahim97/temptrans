...
import { QRCodeVersion, QRCodeElementType, FixMeLater } from './types/types';

@Component({
  selector: 'atlp-qrcode',
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `<div #qrcElement [class]="cssClass"></div>`,
})
export class AtlpQRCodeComponent implements OnChanges {
  ...
  private async createQRCode(): Promise<void> {
    ...
    if (this.version && this.version > 40) {
      console.warn('[atlp-qrcode] max value for `version` is 40');
      this.version = 40;
    } else if (this.version && this.version < 1) {
      console.warn('[atlp-qrcode]`min value for `version` is 1');
      this.version = 1;
    } else if (this.version !== undefined && isNaN(this.version)) {
      console.warn(
        '[atlp-qrcode] version should be a number, defaulting to auto.'
      );
    ...
    try {
      if (!this.isValidQrCodeText(this.qrdata)) {
        throw new Error(
          '[atlp-qrcode] Field `qrdata` is empty, set \'allowEmptyString="true"\' to overwrite this behaviour.'
        );
      }
      ...
      switch (this.elementType) {
        case 'canvas': {
          ...
          this.toCanvas(canvasElement, config)
            ...
            .catch((e) => {
              console.error('[atlp-qrcode] canvas error:', e);
            });
          break;
        }
        case 'svg': {
          ...
          this.toSVG(config)
            ...
            .catch((e) => {
              console.error('[atlp-qrcode] svg error:', e);
            });
          break;
        }
        case 'url':
        ...
        default: {
          ...
          this.toDataURL(config)
            ...
            .catch((e) => {
              console.error('[atlp-qrcode] img/url error:', e);
            });
        }
      }
    } catch (e: FixMeLater) {
      console.error('[atlp-qrcode] Error generating QR Code:', e.message);
    }
  }
  ...
  emitQRCodeURL(element: HTMLCanvasElement | HTMLImageElement | SVGSVGElement) {
    ...
    fetch(urlImage)
      ...
      .catch((error) => {
        console.error(
          '[atlp-qrcode] Error when fetching image/png URL: ' + error
        );
      });
  }
}
