...
export class JsonValidators {
    ...
    return (
      ...
      const isValid = isArray(currentValue)
        ? currentValue.every((inputValue) =>
            allowedValues.some((enumValue) => isEqualVal(enumValue, inputValue))
          )
        : allowedValues.some((enumValue) =>
            isEqualVal(enumValue, currentValue)
          );
      return xor(isValid, invert)
        ? null
  ...
  static format(requiredFormat: JsonSchemaFormatNames): IValidatorFn {
    ...
    return (
      ...
      if (isString(currentValue)) {
        const formatTest: Function | RegExp = jsonSchemaFormat[requiredFormat];
  ...
  static composeAnyOf(validators: IValidatorFn[]): IValidatorFn {
    ...
    return (
      ...
      const arrayOfErrors = _executeValidators(
        control,
        presentValidators,
        invert
      ).filter(isDefined);
  ...
  static composeOneOf(validators: IValidatorFn[]): IValidatorFn {
    ...
    return (
      ...
      const arrayOfValids = _executeValidators(
        control,
        presentValidators,
        invert
      );
