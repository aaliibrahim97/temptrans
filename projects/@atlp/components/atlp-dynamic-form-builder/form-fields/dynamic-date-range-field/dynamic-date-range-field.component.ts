...
@Component({
  selector: 'dynamic-date-range-field',
  templateUrl: './dynamic-date-range-field.component.html',
  styleUrls: ['./dynamic-date-range-field.component.scss'],
})
export class DynamicDateRangeFieldComponent
    ...
    IDynamicFieldComponent
{
  ...
  ngOnInit() {
    ...
    this.group.controls['rangeFormGroup']?.valueChanges.subscribe((value) => {
      this.group.controls[this.field.name].setValue(
        this.group.get('rangeFormGroup').value
      );
    });
  }
