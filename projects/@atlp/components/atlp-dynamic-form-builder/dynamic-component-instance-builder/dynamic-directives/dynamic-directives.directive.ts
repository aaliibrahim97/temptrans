...
@Directive({
  selector: '[atlpDynamicDirectives],[ngComponentOutletatlpDynamicDirectives]',
  providers: [IoFactoryService],
})
export class DynamicDirectivesDirective implements OnDestroy, DoCheck {
  @Input()
  atlpDynamicDirectives: DynamicDirectiveDef<any>[];
  @Input()
  ngComponentOutletatlpDynamicDirectives: DynamicDirectiveDef<any>[];

  @Output()
  atlpDynamicDirectivesCreated = new EventEmitter<DirectiveRef<any>[]>();

  private lastCompInstance: any;

  private get directives() {
    return (
      this.atlpDynamicDirectives || this.ngComponentOutletatlpDynamicDirectives
    );
  }
  ...
  private processDirChanges(
    ...
    if (createdDirs.length) {
      this.atlpDynamicDirectivesCreated.emit(createdDirs.filter(Boolean));
    }
  }
