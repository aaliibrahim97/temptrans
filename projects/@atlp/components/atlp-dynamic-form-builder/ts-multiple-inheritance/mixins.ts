...
function DynamicMultiInheritanceMixin<A extends any[], I1, S1>(
  c1: Class<A, I1, S1>
): Class<A, I1, S1>;

function DynamicMultiInheritanceMixin<
  A1 extends any[],
  I1,
  S1,
  A2 extends any[],
...
function DynamicMultiInheritanceMixin(...constructors: Class[]) {
  ...
  function MixedClass(...args) {
    ...
    if (
      ...
      typeof this[initFunctionName] === 'function'
    )
  ...
  if (settings.decoratorInheritance !== 'none') {
    ...
    applyPropAndMethodDecorators(
      classDecorators?.static ?? {},
      DecoratedMixedClass
    );
    applyPropAndMethodDecorators(
      classDecorators?.instance ?? {},
...
const applyPropAndMethodDecorators = (
  ...
  if (propDecorators)
    for (let key in propDecorators)
      for (let decorator of propDecorators[key]) decorator(target, key);
...
const mix =
  ...
  (decoratedClass) => {
    // @ts-ignore
    const mixedClass = DynamicMultiInheritanceMixin(
      // @ts-ignore
      ...ingredients.concat([decoratedClass])
    );
...
export {
  DynamicMultiInheritanceMixin as MultiInheritanceMixin,
  mix as MultiInheritanceMix,
};
