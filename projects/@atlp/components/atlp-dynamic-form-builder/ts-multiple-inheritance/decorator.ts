...
const mergeObjectsOfDecorators = <
  T extends PropertyDecorator | MethodDecorator
>(
...
const mergePropertyAndMethodDecorators = (
...
const findAllConstituentClasses = (...classes: Class[]): Class[] => {
  ...
  while (frontier.size > 0) {
    for (let clazz of frontier) {
      const protoChainClasses = protoChain(clazz.prototype).map(
      ...
      const newClasses = potentiallyNewClasses.filter(
        (c) => !allClasses.has(c)
      );
...
const decorateMember = <T extends PropertyDecorator | MethodDecorator>(
  decorator: T
): T =>
  ((object, key, ...otherArgs) => {
    const decoratorTargetType =
      typeof object === 'function' ? 'static' : 'instance';
    const decoratorType =
      typeof object[key] === 'function' ? 'method' : 'property';
    const clazz: Class =
      decoratorTargetType === 'static' ? object : object.constructor;

    const decoratorsForClass = getDecoratorsForClass(clazz);

    const decoratorsForTargetType =
      decoratorsForClass?.[decoratorTargetType] ?? {};
