...
export function isRecord(item: any): boolean {
  return !!item && Object.prototype.toString.call(item) === '[object Record]';
}
...
export function toJavaScriptType(value, types, strictIntegers = true) {
  ...
  if (
    inArray('number', types) ||
    (!strictIntegers && inArray('integer', types))
  ) {
    if (isNumber(value, 'strict')) {
      return value;
    }
  ...
  if (
    isDate(value) &&
    (inArray('integer', types) || inArray('number', types))
  ) {
...
export function toSchemaType(value, types) {
  ...
  if (
    (<SchemaPrimitiveType[]>types).includes('boolean') &&
    !isBoolean(value, 'strict')
  ) {
    return value;
  }
