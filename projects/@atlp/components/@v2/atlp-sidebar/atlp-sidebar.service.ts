...
export class AtlpSidebarV2Service {
  // Private
  private _registry: { [key: string]: AtlpSidebarV2Component } = {};
   ...
   */
  getSidebar(key): AtlpSidebarV2Component {
    // Check if the sidebar exists
    if (!this._registry[key]) {
      console.warn(
        `The sidebar with the key '${key}' doesn't exist in the registry.`
  ...
  get allOpenedSidebar(): string[] {
    return Array.from(Object.values(this._registry))
      .filter((sidebar) => {
        return sidebar.opened !== false;
      })
      .sort((prev, next) => {
        return prev.timeOpened - next.timeOpened;
      })
  ...
  /**
   * Return if sidebar open or close
   */
  private _sidebarSubject = new BehaviorSubject<boolean>(false);
  isSideBarOpen$ = this._sidebarSubject.asObservable();

  changeSideBarOpenStatus(isOpen) {
    this._sidebarSubject.next(isOpen);
  }
}
